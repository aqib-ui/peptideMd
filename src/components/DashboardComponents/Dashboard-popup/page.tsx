...
//   onClose: () => void;
//   // onSubscribe: () => void;
//   onSubscribe: (plan: "annual" | "monthly") => void; // Add plan parameter
// }
...
interface Props {
  onClose: () => void;
  onSubscribe: (plan: "annual" | "monthly") => void; // Updated to accept plan argument
}
...
export default function DashboardPopup({ onClose, onSubscribe }: Props) {
  ...
  const handleSubscribe = () => {
    if (!selectedPlan) return;
    onSubscribe(selectedPlan); // Pass selectedPlan to onSubscribe
  };
